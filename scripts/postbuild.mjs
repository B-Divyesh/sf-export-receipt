...
const emitted = Object.values(manifest).flatMap((asset) => [asset.file, ...(asset.css || [])]);
const assets = [...new Set(['/', '/index.html', '/manifest.webmanifest', '/favicon.svg', '/icon-192.png', '/icon-512.png', '/offline.html', ...emitted.map((file) => `/${file}`)])];
